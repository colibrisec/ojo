...
!!! note "Cross-resource Terraform checks see one directory plus one level of local modules, not the whole module graph"
    All `.tf` files in the same directory are scanned together — the AWS S3 checks that correlate a bucket with its `aws_s3_bucket_versioning`/`..._logging`/`..._public_access_block`/`..._server_side_encryption_configuration` resources (and the VPC/flow-log check) match references across files in that directory. A `module "x" { source = "./..." }` block with a literal, local (relative-path) source is also followed one level deep — if the module's own resources reference the parent's via a passed-in variable (`bucket = var.bucket_id` where the module call has `bucket_id = aws_s3_bucket.data.id`), that's resolved back to the parent resource. What's still out of reach: a non-local (registry/git) module source, a module nested inside another module, and general module input/output resolution beyond this one direct-reference pattern. Azure and GCP checks are all single-resource (no cross-resource correlation) for now.
